...
def _GetCommand(driver, instances):
  if (
    len(instances.instance)
    and instances.instance[0].device.name
    == _env.OclgrindOpenCLEnvironment().name
  ):
...
def Drive(
  instances: cldrive_pb2.CldriveInstances, timeout_seconds: int = 300
) -> cldrive_pb2.CldriveInstances:
